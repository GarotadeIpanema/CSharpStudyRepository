...
{
    public partial class ObjToStrCastring : Form
    {
        public ObjToStrCastring()
        {
            InitializeComponent();
            ...
            // Object -> String
            object[] obj = dt.Select().Select(x => x["TestStr1"]).ToArray();
            string[] teststr = Array.ConvertAll(obj, p => (p ?? String.Empty).ToString());
            textBox1.Text = obj[0].ToString();
            textBox2.Text = teststr[0];
        }
    }
}
